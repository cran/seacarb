...
#       Std definitions needed to comput buffer factors
	Alkc = (2*CO3 + HCO3)
	Borate  = (bor / (1 + h/Kb))   # could also use equivalent formulation of Borate = bor * Kb / (Kb + h)
	oh = Kw / h
...
#       - later modified to comply w/ formulas in Excel sheet of Chris Sabine (23 Aug 2010)
	#Segle   = (HCO3 + 4*CO3 + (h*Borate/(Kb + h)) + h - oh)  #Formula from Table 1 (Egleston et al.)         !Incorrect (inverse last sign)
	 Segle   = (HCO3 + 4*CO3 + (h*Borate/(Kb + h)) + h + oh)  #Formula from Sabine (Excel sheet, 23 Aug 2010) !Correct
	 Pegle  = (2*CO2 + HCO3)                                  #Formula from Table 1 (Egleston et al.)         !Correct
 	#Qegle  = (HCO3 - (h*Borate/(Kb + h)) - h - oh)           #Formula from Sabine (Excel sheet, 23 Aug 2010) !Correct
 	 Qegle  = 2*Alkc - Segle                                  #Formula from derivation by J. Orr (same result as line just above) !Correct
#
...
#                - to convert to mmol/kg (units shown by Egleston), multiply each factor by 1000 after calling this routine
	...
	#omegaDIC = ( DIC - (Alkc*Pegle/HCO3) )                   #INCORRECT - Formula from Table 1 (Egleston et al.) - replace HCO3 by Qegle
	#omegaALK = (Alkc - DIC*HCO3/Pegle)                       #INCORRECT - Formula from Table 1 (Egleston et al.) - replace HCO3 by Qegle
	omegaDIC = ( DIC - (Alkc*Pegle/Qegle) )                   #CORRECT   - Formula from Sabine (Excel sheet, 23 Aug 2010)
	omegaALK = ( Alkc - DIC*Qegle/Pegle )                     #CORRECT   - Formula from Sabine (Excel sheet, 23 Aug 2010)
